...
class Data(object):
    ...
    def __init__(self):
        ...
        self.expert_advice = False #flag for expert_advice
        self.expertAdviceClauses = {} #expert advice clauses stored here
        self.multiple_advice = False #flag for multiple advice
        self.multipleAdviceClauses = {} #multipleadviceclauses stored here
        self.facts = [] #facts
        self.facts_in_bk = []
...
class Utils(object):
    ...
    @staticmethod
    def setTrainingData(target=None,facts=None,examples=None,pos=None,neg=None,bk=None,regression=False,sampling_rate=None,advice=False,expert_advice=False,multiple_advice=False):
        '''sets facts, examples and background'''
        Utils.data = Data()
        Utils.data.advice = advice
        Utils.data.expert_advice = expert_advice
        Utils.data.multiple_advice = multiple_advice
        if expert_advice:
            with open("Dem/train/expert_advice.txt") as fp: #read expert advice from train folder
                expertAdviceFileLines = fp.read().splitlines()
                for line in expertAdviceFileLines:
                    expertAdviceClause = line.split(' ')[0]
                    Utils.data.expertAdviceClauses[expertAdviceClause] = {}
                    preferredTargets = line.split(' ')[1][1:-1].split(',')
                    if preferredTargets[0]:
                        Utils.data.expertAdviceClauses[expertAdviceClause]['preferred'] = preferredTargets
                    elif not preferredTargets[0]:
                        Utils.data.expertAdviceClauses[expertAdviceClause]['preferred'] = []
                    nonPreferredTargets = line.split(' ')[2][1:-1].split(',')
                    if nonPreferredTargets[0]:
                        Utils.data.expertAdviceClauses[expertAdviceClause]['nonPreferred'] = nonPreferredTargets
                    elif not nonPreferredTargets[0]:
                        Utils.data.expertAdviceClauses[expertAdviceClause]['nonPreferred'] = []
                        
        if multiple_advice:
            with open("Dem/train/multiple_advice.txt") as fp: #read advice from train folder
                adviceFileLines = fp.read().splitlines()
                for line in adviceFileLines:
                    adviceClause = line.split(' ')[0] #get advice clause
                    Utils.data.multipleAdviceClauses[adviceClause] = {}
                    preferredTargets = line.split(' ')[1][1:-1].split(',')
                    if preferredTargets[0]:
                        Utils.data.multipleAdviceClauses[adviceClause]['preferred'] = preferredTargets
                    elif not preferredTargets[0]:
                        Utils.data.multipleAdviceClauses[adviceClause]['preferred'] = []
                    nonPreferredTargets = line.split(' ')[2][1:-1].split(',')
                    if nonPreferredTargets[0]:
                        Utils.data.multipleAdviceClauses[adviceClause]['nonPreferred'] = nonPreferredTargets
                    elif not nonPreferredTargets[0]:
                        Utils.data.multipleAdviceClauses[adviceClause]['nonPreferred'] = []
        if advice:
            with open("Dem/train/advice.txt") as fp: #read advice from train folder
                adviceFileLines = fp.read().splitlines()
                for line in adviceFileLines:
                    adviceClause = line.split(' ')[0] #get advice clause
    ...
    def setTestData(target=None,facts=None,pos=None,neg=None,examples=None,regression=False):
        ...
        if not regression:
            testData.setPos(pos,target)
            testData.setNeg(neg,target)
        elif regression:
